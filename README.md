...
Fund Tracking 是一个基金跟踪项目，前端提供基金列表、搜索、详情、登录注册和 watchlist 管理，后端提供基金数据读取、用户认证、关注列表、行情更新和提醒任务接口。项目当前主链路为 Next.js 前端、Go `net/http` 后端、Redis 缓存层、MongoDB Atlas 数据库，并通过 GitHub Actions 定时触发后端更新任务。
...
| 后端 | Go 1.26、标准库 `net/http`、MongoDB Go Driver、`go-redis/v9` |
| 数据库 / 缓存 | MongoDB Atlas、Redis、Cache Aside Pattern |
...
```text
Browser -> Vercel Next.js Frontend -> Render Go API -> Redis cache -> MongoDB Atlas
```

Redis 当前用于基金详情接口的 Cache Aside 缓存层，不是主数据库；MongoDB Atlas 仍然是主数据源。

## 核心功能

- 基金列表展示：`GET /api/funds`
- 基金搜索：`GET /api/search_proxy?query=...`，同时保留 `GET /api/funds/search?query=...`
- 基金详情：`GET /api/fund/{fundCode}`，使用 Redis 缓存基金详情，并通过 `X-Cache` 响应头区分 `HIT` / `MISS`
- 登录注册：邮箱注册、登录、JWT 鉴权、邮箱验证码验证和重发
- Watchlist：登录用户可以添加、查看、删除关注基金，并修改提醒阈值
- 未收录基金导入：登录后可导入合法的 6 位基金代码
- 定时更新：GitHub Actions 工作日触发基金行情更新、阈值检查和提醒邮件发送；`/api/update` 成功更新基金后，会根据 `updated_codes` 主动失效对应的 `fund:detail:{fundCode}` 缓存
...
11. MongoDB 更新成功后，根据 `updated_codes` 对应删除 Redis 中的 `fund:detail:{fundCode}` 缓存；接口不直接重建缓存，下次基金详情请求 Redis `MISS` 后再从 MongoDB 读取并写回 Redis。
...
| `REDIS_ADDR` | Redis 地址，默认本地为 `127.0.0.1:6379` |

本地开发可以用 Docker 启动 Redis：
