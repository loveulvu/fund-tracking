...
系统支持基金列表和详情、已收录基金搜索、注册登录、注册邮箱 6 位验证码、JWT 鉴权、watchlist、阈值提醒、未收录 6 位基金代码导入、基金元数据补全、阶段收益补全，以及 GitHub Actions 工作日自动执行基金更新、提醒检测和 Resend 邮件发送。
...
UI 已从旧的黑色页面统一成浅色金融 dashboard 风格。注册页支持邮箱 6 位验证码输入和重发。基金列表页支持搜索已收录基金，如果用户输入完整 6 位基金代码且数据库未收录，会显示导入入口。
...
- `users`：保存用户邮箱、密码哈希、邮箱验证状态和验证码 hash 等。
- `watchlists`：保存用户关注基金，包含 `userId`、`fundCode`、`fundName`、`alertThreshold` 等。
- `alert_logs`：保存提醒触发和邮件发送状态，用于防重复提醒。

基金基础行情更新使用 `UpdateOne + $set + upsert`，以 `fund_code` 匹配。元数据和阶段收益补全只 `$set` 有效非空字段，不会把已有字段覆盖为空，也不会把缺失阶段收益写成 0。

### 6. JWT 登录认证

注册时使用 bcrypt 保存密码哈希，并发送 6 位数字邮箱验证码。验证码 10 分钟有效，数据库只保存 HMAC-SHA256 hash，不保存明文验证码。新注册用户未验证邮箱时登录会返回 403；老用户缺失 `emailVerified` 字段时按已验证处理，避免历史账号被突然阻断。

登录成功后签发 JWT，前端后续请求在 `Authorization: Bearer <token>` 中携带 token。
...
另一个问题是部分阶段收益字段缺失时被显示成 `0.00%`。根因是 Go 结构体使用 `float64`，MongoDB 缺失字段解码后变成 0。修复方式是把可缺失阶段收益字段改成可空值，前端区分 null 和真实 0。后续新增独立 `/api/funds/performance` 接口补全阶段收益，但仍保持缺失值不写 0。

邮件提醒方面，我把提醒检测和邮件发送拆成两个接口。`/api/alerts/check` 只负责扫描阈值和写入 `alert_logs`，`/api/alerts/send` 负责发送待处理邮件。`alert_logs` 通过状态机和去重键避免重复发送。

旧 Flask / Python 代码已经移动到 `archive/legacy-python/`，只作为迁移参考，不参与当前线上主链路。
...
12. **注册邮箱 6 位验证码**
    使用 Resend 发送验证码，HMAC-SHA256 存 hash，支持 10 分钟过期、5 次错误限制和 60 秒重发冷却。

13. **阶段收益独立补全接口**
    `POST /api/funds/performance` 独立于 `/api/update`，只写有效阶段收益和 `performanceUpdatedAt`，暂不接入 GitHub Actions。
...
### 15. 注册邮箱验证码为什么用 hash 存库？

验证码本质上也是短期凭证，如果明文存库，数据库泄漏时可以直接被使用。当前用 HMAC-SHA256 保存验证码 hash，HMAC key 复用后端 `JWT_SECRET`，验证时只比较 hash。

### 16. 阶段收益接口为什么没有立刻接入 GitHub Actions？

它依赖额外外部接口。当前手动调用和公开 API 读取已经验证可用，但为了不影响稳定的 update/check/send 提醒链路，暂时不把它接入定时 workflow。

### 17. 当前最大不足是什么？

持仓和收益曲线还没有真实数据支撑。watchlist 只是关注列表，不等于持仓系统。后续如果要做组合收益，需要交易记录、持仓份额和历史净值快照。
...
1. Render Free 可能冷启动。
2. 阶段收益接口已实现，但暂未接入 GitHub Actions，仍需继续观察外部接口稳定性。
3. 持仓金额、收益曲线和资产配置未实现。
4. 不支持真实交易记录系统。
5. 不支持全市场模糊搜索自动外部导入。
6. 移动端 UI 仍可继续优化。
7. 旧 Flask / Python 代码已归档到 `archive/legacy-python/`，仅作为迁移参考。

后续计划：

1. 设计 `go-backend-api-version` 合并到 `main` 的 PR 和部署验证策略。
2. 设计持仓和收益曲线的数据模型。
...
- 将基金跟踪系统核心后端从 Flask 增量迁移到 Go，基于 `net/http`、MongoDB Go Driver、JWT、bcrypt 和邮箱验证码实现基金查询、认证、watchlist、导入、更新和提醒接口，并保持前端 API 兼容。
- 设计基金数据更新和补全链路，支持 `defaultFundCodes + distinct(watchlists.fundCode)` 自动更新、6 位基金代码手动导入、元数据补全、阶段收益补全，并在外部数据源失败时避免写入无效数据。
- 搭建 Vercel 前端、Render Go 后端、MongoDB Atlas、GitHub Actions 和 Resend 的线上链路，实现工作日自动 update/check/send，支持阈值提醒邮件、防重复发送、注册验证码邮件和 artifact 可观测性。
